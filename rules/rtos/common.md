...
- Verification: Build a call-context table from the selected runtime documentation and inspect all direct and indirect call paths.
...
- Verification: Review every wait value and result branch, then test wake-up, timeout, cancellation, and resource-unavailable paths.
...
- Verification: Draw the lifecycle and handoff graph, inspect all failure paths, and test shutdown concurrent with pending work.
...
- Verification: Review the priority/resource graph and measure the worst-case blocking interval under the configured policy.
...
- Verification: Measure or calculate high-water use with the target build, exercise deepest paths, and compare it with the configured bound and margin.
...
- Verification: Test shutdown with queued work, blocked waiters, pending callbacks, timeout, and repeated stop requests.
